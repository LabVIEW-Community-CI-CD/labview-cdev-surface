...
- `.github/workflows/ops-slo-gate.yml` must enforce deterministic SLO gate policy using `scripts/Test-OpsSloGate.ps1`.
- SLO gate reason codes must remain explicit:
...
- `.github/workflows/release-rollback-drill.yml` must run `scripts/Invoke-ReleaseRollbackDrill.ps1`.
- Rollback drill reason codes must remain explicit:
