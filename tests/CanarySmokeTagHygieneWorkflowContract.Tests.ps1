...
Describe 'Canary smoke tag hygiene workflow contract' {
    ...
    It 'is scheduled and dispatchable with apply-controls inputs' {
        ...
        $script:workflowContent | Should -Match 'tag_family'
        $script:workflowContent | Should -Match 'legacy_date_window'
        $script:workflowContent | Should -Match 'semver'
    ...
    It 'enforces keep-latest canary tag cleanup behavior' {
        ...
        $script:scriptContent | Should -Match "ValidateSet\('auto', 'legacy_date_window', 'semver'\)"
        $script:scriptContent | Should -Match 'KeepLatestN'
        $script:scriptContent | Should -Match 'SemverCanaryTagRegex'
        $script:scriptContent | Should -Match 'tag_family_mode'
        $script:scriptContent | Should -Match 'legacy_candidate_count'
        $script:scriptContent | Should -Match 'semver_candidate_count'
        $script:scriptContent | Should -Match 'migration_warnings'
