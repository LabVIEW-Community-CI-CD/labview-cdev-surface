...
Describe 'Scope A ops runbook contract' {
    ...
    It 'documents deterministic incident commands for runner, sync-guard, and canary hygiene' {
        ...
        $script:runbookContent | Should -Match 'workflow-bot-token-drill\.yml'
        $script:runbookContent | Should -Match 'ops-policy-drift-check\.yml'
        $script:runbookContent | Should -Match 'release-guardrails-autoremediate\.yml'
        ...
        $script:runbookContent | Should -Match 'release-race-hardening-gate\.yml'
        $script:runbookContent | Should -Match 'branch-protection-drift-check\.yml'
        $script:runbookContent | Should -Match 'Invoke-ReleaseRaceHardeningDrill\.ps1'
        $script:runbookContent | Should -Match 'Invoke-ReleaseGuardrailsSelfHealing\.ps1'
        $script:runbookContent | Should -Match 'Test-ReleaseRaceHardeningGate\.ps1'
        $script:runbookContent | Should -Match 'Set-ReleaseBranchProtectionPolicy\.ps1'
        $script:runbookContent | Should -Match 'Test-ReleaseBranchProtectionPolicy\.ps1'
        ...
        $script:runbookContent | Should -Match 'Release Race Hardening Drill Alert'
        $script:runbookContent | Should -Match 'Branch Protection Drift Alert'
        $script:runbookContent | Should -Match 'Release Guardrails Auto-Remediation Alert'
        $script:runbookContent | Should -Match 'Workflow Bot Token Health Alert'
        $script:runbookContent | Should -Match 'release-race-hardening-weekly-summary\.json'
    ...
    It 'keeps README and AGENTS aligned to Scope A workflows' {
        ...
        $script:readmeContent | Should -Match 'workflow-bot-token-drill\.yml'
        ...
        $script:agentsContent | Should -Match 'workflow-bot-token-drill\.yml'
        ...
        $script:agentsContent | Should -Match 'Release Race Hardening Drill'
