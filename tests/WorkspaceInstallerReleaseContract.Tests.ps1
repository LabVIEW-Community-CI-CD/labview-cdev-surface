...
Describe 'Workspace installer release workflow contract' {
    ...
    It 'defines package and publish jobs with release asset upload' {
        ...
        $script:workflowContent | Should -Match 'installer_contract\.cli_bundle'
        $script:workflowContent | Should -Match 'workspace-governance-payload/tools/cdev-cli'
        $script:workflowContent | Should -Match 'CLI bundle Windows hash mismatch'
        $script:workflowContent | Should -Match 'CLI bundle Linux hash mismatch'
