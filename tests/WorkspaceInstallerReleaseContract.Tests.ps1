...
Describe 'Workspace installer release workflow contract' {
    ...
    It 'keeps dispatch-only wrapper and forwards to release core workflow' {
        ...
        $script:wrapperWorkflowContent | Should -Match 'concurrency:'
        $script:wrapperWorkflowContent | Should -Match 'group:\s*release-workspace-installer-\$\{\{\s*github\.repository\s*\}\}-\$\{\{\s*inputs\.release_tag\s*\}\}'
        $script:wrapperWorkflowContent | Should -Match 'cancel-in-progress:\s*false'
